...
int send_string(int connection_fd, char *s) {
    ...
    ssize_t result = send(connection_fd, s, len, NULL);
    return result > 0 ? 0 : result;
}

int send_uint32(int connection_fd, uint32_t x) {
    uint32_t conv = htonl(x);
    ssize_t result = send(connection_fd, &conv, sizeof(conv), NULL);
    return result > 0 ? 0 : result;
}

int receive_string(int connection_fd, char *s, uint32_t *len) {
    receive_uint32(connection_fd, len);
    for (int i = 0; i < len; i++) {
        receive_byte(connection_fd, (uint8_t *)(s + i));
    }
}

int receive_uint32(int connection_fd, uint32_t *x) {
    ssize_t result = recv(connection_fd, x, sizeof(x), NULL);
    return result > 0 ? 0 : result;
}
